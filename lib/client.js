window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			'.dshvs-optSess { font-size:12px; opacity:.95; word-break:break-all; padding-left:22px; }',
		].join('\n');
		...
		function apply(ctx) {
			...
			// —— 会话标题（弹窗判别用）——
			// 会话行展示名取 displayTitle（持久化标题 → 项目目录名 → 会话 id 兜底），
			// blank 行是「新建会话」占位，不计入。
			function sessionTitleOf(row) {
				if (!row) return '';
				return String(row.displayTitle || row.title || row.name || row.id || '').trim();
			}
			function sessionRowsForFolder(folder) {
				...
				try {
					...
					for (const id of Object.keys(byId)) {
						...
						if (norm(row.cwd) !== norm(folder)) continue;
						out.push({ id, title: sessionTitleOf(row) || '未命名会话' });
					}
				} catch (e) { /* 会话服务不可用：弹窗退回只显示目录 */ }
				return out;
			}
			function describeSessions(folder) {
				const rows = sessionRowsForFolder(folder);
				if (rows.length === 0) return '';
				const names = rows.slice(0, 2).map((r) => '「' + r.title + '」');
				return rows.length > 2 ? (names.join('、') + ' 等 ' + rows.length + ' 个会话') : names.join('、');
			}
			...
			function showEvictPrompt(target, limit) {
				...
				const targetSess = describeSessions(target);
				desc.textContent = '打开「' + baseName(target) + '」' + (targetSess ? '（' + targetSess + '）' : '') +
					'需要新实例，实例池已满（上限 ' + limit +
				...
				victims.forEach((inst, i) => {
					...
					sess.className = 'dshvs-optSess';
					const sessText = describeSessions(inst.folder);
					sess.textContent = sessText ? ('会话：' + sessText) : '（该空间暂无会话记录）';
			...
			function bootInst(inst, urlBase) {
				...
				// 主题与专注布局都只在 workbench 启动时读取一次：先落盘、后启动 iframe
				const p = Promise.all([pushTheme({ force: true, booting: true }), pushLayout({ force: true, booting: true })]);
			...
			// —— 专注布局（0.1.21）：新开的 VS Code 默认只显示 Codex（副边栏最大化） ——
			// —— 信任目录（0.1.22）：security.workspace.trust.enabled=false，禁用 Restricted Mode，
			//     否则 Codex 等扩展在未信任目录下被禁用（信任开关取宿主 config.trustWorkspace）。
			// 生效时机与主题一致：workbench 只在启动时读一次 user settings，因此落盘先于
			// iframe 启动（bootInst 前置推送）；开关变化（设置页保存 / 状态轮询差值，含多页签
			// 他端改动）对池内全部已加载实例收敛重载。注意：收敛只做重载、不追求把已打开
			// 实例拉回专注——workspace 布局由 VS Code 自己按 workspace 持久化并接管
			// （defaultVisibility 仅首次打开生效；用户解除最大化后保持其布局，不被强拉）。
			let focusLayoutOn = true;
			let trustOn = true;
			let lastPushedLayout = null;
			function convergeLayout() {
				for (const inst of Array.from(pool.values())) {
					if (!inst.loaded || !inst.base) continue;
					inst.themeBump += 1;
					inst.frame.src = inst.base + '&dshreload=' + inst.themeBump;
			...
			const pushLayout = (opts) => {
				const force = !!(opts && opts.force);
				const booting = !!(opts && opts.booting);
				const want = { focus: focusLayoutOn, trust: trustOn };
				if (!force && lastPushedLayout && lastPushedLayout.focus === want.focus && lastPushedLayout.trust === want.trust) return null;
				const p = api('/dsh-vscode/layout', { method: 'POST', body: JSON.stringify({ focus: want.focus }) }).then((r) => {
					lastPushedLayout = want;
					if (r && r.changed && !booting) convergeLayout();
					return r;
				}, () => null);
				return p;
			};
			...
			const setValue = (v) => {
				if (v && v.config) {
					themeFollowOn = v.config.themeFollow !== false;
					// 专注布局 / 信任目录 开关差值：配置变化（本页设置页或多页签他端）即推送，changed 才收敛重载
					const fl = v.config.focusLayout !== false;
					const tw = v.config.trustWorkspace !== false;
					if (fl !== focusLayoutOn || tw !== trustOn) { focusLayoutOn = fl; trustOn = tw; pushLayout(); }
				}
			...
			function VscodeSettings(props) {
				...
				const doSave = () => {
					props.saveConfig({
						...
						focusLayout: draft.focusLayout !== false,
						trustWorkspace: draft.trustWorkspace !== false,
						extraArgs: String(draft.extraArgsText || '').split(/\s+/).filter((x) => x.length > 0),
					}).then(() => { setSaved('已保存'); pushTheme(); pushLayout(); return null; }).catch(() => {});
				};
				...
				field('专注布局（新开的 VS Code 默认只显示 Codex）', React.createElement('input', { type: 'checkbox', checked: draft.focusLayout !== false, onChange: (e) => upd('focusLayout', e.target.checked) })),
				field('信任打开的目录（关闭 Restricted Mode）', React.createElement('input', { type: 'checkbox', checked: draft.trustWorkspace !== false, onChange: (e) => upd('trustWorkspace', e.target.checked) })),
				field('附加参数（空格分隔）', React.createElement('input', { className: 'dshvs-input', value: draft.extraArgsText || '', placeholder: '例如 --disable-get-started-banner', onChange: (e) => upd('extraArgsText', e.target.value) })),
					React.createElement('div', { className: 'dshvs-kv' }, '「打开目录跟随当前空间」开启时，VS Code 打开当前会话所在空间（会话 cwd）的目录；关闭或会话无目录时使用「工作区路径」。「实例池上限」按 workspace 缓存 VS Code 实例（默认 2）：同空间会话互切不重载，跨空间切换秒切；池满时弹窗选择要踢出的实例。「专注布局」开启时新打开的 VS Code 默认只显示右侧 Codex（主边栏/面板/编辑器区收起，活动栏保留），状态栏与欢迎页隐藏；在 VS Code 内解除最大化/打开边栏即恢复完整布局且不被强拉，再点副边栏的「最大化」按钮可回到专注。「信任打开的目录」写入 security.workspace.trust.enabled=false 禁用 Restricted Mode（否则 Codex 等扩展在未信任目录下被禁用）；桥只会打开本机 DSH 空间目录。改端口/目录/来源后需重启 code-server 生效；配置持久化在工作区 .dsh/vscode-bridge/config.json。'),
