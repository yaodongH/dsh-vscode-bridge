	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const pushLayout = (opts) => {
				...
				const focus = focusLayoutOn;
				if (!force && focus === lastFocusApplied) return null;
				const p = api('/dsh-vscode/layout', { method: 'POST', body: JSON.stringify({ focus: focus }) }).then((r) => {
					lastFocusApplied = focus;
			...
			const setValue = (v) => {
				if (v && v.config) {
					...
					// 专注布局开关差值：配置变化（本页设置页或多页签他端）即推送，changed 才收敛重载
					const fl = v.config.focusLayout !== false;
					if (fl !== focusLayoutOn) { focusLayoutOn = fl; pushLayout(); }
				}
