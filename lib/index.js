...
export function apply(ctx) {
  ...
  const WEB_BUNDLE_EXTRAS = {
    // markdown 扩展的语言服务 worker 由 extension.js 运行期动态 importScripts，
    // 静态 import 解析扫不到，需要显式列出。
    'markdown-language-features': ['dist/browser/serverWorkerMain.js'],
  }

  async function ensureBuiltinWebBundles(installDir) {
    const libDir = installDir + '/lib/vscode'
    const extRoot = libDir + '/extensions'
    if (!existsSync(extRoot + '/package.json')) return 0
    const marker = installDir + '/web-bundles-patch.json'
    try {
      const m = JSON.parse(readFileSync(marker, 'utf8'))
      if (m.done === true && m.vscodeCdnCommit === PIN.vscodeCdnCommit) return 0
    } catch (e) { /* 无标记 → 首次执行 */ }
    ...
    writeFileSync(marker, JSON.stringify({
      done: failures.length === 0, vscodeCdnCommit: PIN.vscodeCdnCommit, files: downloaded,
