  ...
  const routeHandler = async (req, res) => {
    ...
    try {
      ...
      if (url === '/dsh-vscode/layout' && req.method === 'POST') {
        ...
        // 专注布局（0.1.21）：把三键写入/移除出 code-server 的 user settings.json。
        ...
        try { cur = JSON.parse(readFileSync(settingsPath, 'utf8')) } catch (e) { cur = {} }
        let changed = false
        for (const k of Object.keys(focusKeys)) {
          if (focus) {
            if (cur[k] !== focusKeys[k]) { cur[k] = focusKeys[k]; changed = true }
        ...
        return jsonOf(res, 200, { ok: true, focus, changed })
      }
